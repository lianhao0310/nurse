...
- 所有文字使用简体中文。`;
   ...
   */
  const CONSULT_SYSTEM = `你是一名严谨、贴心的"Nurse"健康助手。用户会提供：①本次问诊的医嘱文字（可能来自录音转写），②可选的检查结果照片，③可选的处方药照片。
...
- 不编造信息，缺则留空或返回空数组。所有文字使用简体中文。`;
   ...
   * 返回 { diet[], taboo[], text }
   */
  const ADVICE_SYSTEM = `你是一名"Nurse"健康助手。用户会提供：①本次医生医嘱，②历次检查指标趋势，③当前正在服用的药物。
...
- 不编造信息，缺则留空或返回空数组。简体中文`;
  ...
  async function _parseLocal(opts) {
    ...
    var text = await llm.generate({
      ...
      contextLength: lm.contextLength || 1024,
      maxTokens: 512,
      temperature: 0.2,
    });
  ...
  async function _chatStreamLocal(messages, localModel, onChunk, options) {
    ...
    var ctxLen = localModel.contextLength || 1024;
