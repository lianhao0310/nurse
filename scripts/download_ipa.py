...
# -*- coding: utf-8 -*-
"""
下载 GitHub Actions 构建的 nurse iOS .ipa 产物到本地 builds/ 目录。

- 在 GitHub 上查找 build-ios.yml 最近一次「成功」的运行
- 定位名为 nurse-ios-ipa 的 artifact，下载并解压
- 幂等：已下载过的 run 会跳过（通过 builds/run-<id>/.downloaded 标记）
- 鉴权：优先环境变量 GITHUB_PAT，其次仓库根目录 .ipa_config 里的 GITHUB_PAT=

用法：
  GITHUB_PAT=xxx python scripts/download_ipa.py
  python scripts/download_ipa.py        # 从 .ipa_config 读取
"""
import os
import sys
import json
...
def main():
    ...
    for run in runs:
        ...
        print(f"[完成] -> {dest}")
        print(f"        ipa: nurse-{head}.ipa")
        return

    print("最近的构建运行均没有可下载的 ipa 产物（可能尚未配置签名并成功导出）。")
